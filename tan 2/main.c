...
// settings
const unsigned int SCR_WIDTH = 1096;
const unsigned int SCR_HEIGHT = 822;
...
//    #ifdef CGLM_USE_SIMD
//       printf("CGLM SIMD 优化已启用\n");
//   #else
//       printf("CGLM SIMD 优化未启用\n");
//   #endif
//    
//#ifdef CGLM_USE_NEON
//   printf("CGLM SIMD 优化已启用\n");
...
//    Shader cubeShadow=IniSha("tan/vshadow.vs", "tan/fshadow.fs");
//    Shader snakeShadow=IniSha("tan/vshadow_snake.vs", "tan/fshadow_snake.fs");
//    Shader foodShadow=IniSha("tan/vshadow_food.vs", "tan/fshadow_food.fs");
...
//    script(-1, "SNAKE", 0.2, D(u(70)), B, C, 0.3, 0.7, _snakeV*0.5, 0.05);
    ...
    while(!glfwWindowShouldClose(window))
    {
        while (!glfwWindowShouldClose(window)) {
            ...
            //            processInput(window);
            //
            //            if(!Orth)glm_perspective(glm_rad(45.0f), (float)SCR_WIDTH/(float)SCR_HEIGHT, near, far, Proj);
            ...
            //            updateLight(&camera,&simpleDepthShader,&simpleDepthShader_SR, &VBO_SR[1], &VBO_SR[2], &VBO_SR[3], &VBO_SR[4], Snake_diffuse_textureU, Rot_diffuse_textureU, specular_textureU,&VAO_SR_depth, &f, &VBO_F[1], &VAO_F_depth, &foodN,&cubeVAO_depth,&cubeShader,cubeVAO);
            ...
            //        if (glfwGetWindowAttrib(window, GLFW_FOCUSED))printf("n\n");
            ...
            while (STATE<=0&&!glfwWindowShouldClose(window)) {
                ...
                if(snakeH.up!=B&&STATE==-2){
                    if(nHd->others!=NULL){
                        ...
                        Npc[0]=iniNPC(0, not->s, npc_fc, npc_rc, npc_oc, npc_sc, npc_randm, npc_ts, npc_bs, npc_es_sl, npc_apAdv, npc_rps, npc_perFrame, npc_rigor);
                        not->s->ID=1; not=not->others;
                        Npc[1]=iniNPC(1, not->s, npc_fc, npc_rc, npc_oc, npc_sc, npc_randm, npc_ts, npc_bs, npc_es_sl, npc_apAdv, npc_rps, npc_perFrame, npc_rigor);
                        not->s->ID=2; not=not->others;
                        Npc[2]=iniNPC(2, not->s, npc_fc, npc_rc, npc_oc, npc_sc, npc_randm, npc_ts, npc_bs, npc_es_sl, npc_apAdv, npc_rps, npc_perFrame, npc_rigor);
                        not->s->ID=3; not=not->others;
                        Npc[3]=iniNPC(3, not->s, npc_fc, npc_rc, npc_oc, npc_sc, npc_randm, npc_ts, npc_bs, npc_es_sl, npc_apAdv, npc_rps, npc_perFrame, npc_rigor);
                        not->s->ID=4; not=not->others;
                        Npc[4]=iniNPC(4, not->s, npc_fc, npc_rc, npc_oc, npc_sc, npc_randm, npc_ts, npc_bs, npc_es_sl, npc_apAdv, npc_rps, npc_perFrame, npc_rigor);
                        not->s->ID=5;
                        noteRelease_ID(-3);
                        //                    snakeCrush(Npc[0]->H);
                        //                    snakeCrush(Npc[1]->H);
                ...
                if((!Orth||STATE==-2)&&kkk<kkk_max){
                    //                printf("kkk:%f\n",kkk);
                    glm_perspective(glm_rad(kkk), (float)SCR_WIDTH/(float)SCR_HEIGHT, near, far, Proj);
                    //                glm_perspective(glm_rad(kkk), (float)SCR_WIDTH/(float)SCR_HEIGHT, near, far, Proj);
                    camera.distance=cube->size*(1-2*cube->margin)/4*3/2/tanf(glm_rad(kkk/2))+cube->size/2;
                    ...
                    glm_ortho(-cube->size*(0.5-cube->margin), cube->size*(0.5-cube->margin), (-cube->size*(0.5-cube->margin))/4*3, (cube->size*(0.5-cube->margin))/4*3, -100, 100, Proj);
                }
                ...
                if(iniTimer==50){
                    ...
                    char ta[13];
                    sprintf(ta, "%012ld",score.score%10000000000000);
//                    printf("%s\n",ta);
                    script(-4, ta, 0.1, D(u(20)), B, C, 0.6, 0.7, _snakeV, 0.1);
                    sprintf(ta, "%012ld",score.length%10000000000000);
...
//                    printf("%s\n",ta);
    ...
    GameScores newScore={snakeH.score,endLength};
...
void processInput(GLFWwindow *window)
{
    if(glfwGetKey(window, GLFW_KEY_ESCAPE) == GLFW_PRESS||ppp==1){
        ...
        debug(main477)
        noteClean();
        while (snakeDel(snakeH.others)!=-1);
        rotFree();
...
//        printf("mouse\n");
    ...
    if(STATE<=0){
        float x = (xposIn/SCR_WIDTH-0.5)*cube->size;
        float y = -(yposIn/SCR_HEIGHT-0.5)*cube->size/SCR_WIDTH*SCR_HEIGHT;
            mouseLight(x,y);
    }
...
//    printf("(%f,%f)\n",xpos/SCR_WIDTH,ypos/SCR_HEIGHT);
    if(STATE<=0){
        float x = (xpos/SCR_WIDTH-0.5)*cube->size;
        float y = -(ypos/SCR_HEIGHT-0.5)*cube->size/SCR_WIDTH*SCR_HEIGHT;
            mouseLight(x,y);
    }
}
